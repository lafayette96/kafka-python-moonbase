...
for msg in consumer:
    ...
    if data['previousZoneId']:
        ...
        last_seen[random.choice(names_list)] = {
        ...
        if data['carId'] in last_seen:
            print(f"  - This rover was last seen in {last_seen[random.choice(names_list)]['last_zone']} trip took {time.time()-last_seen[data['carId']]['timestamp']}")
